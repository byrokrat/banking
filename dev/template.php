 ...
 */
class <?= $data['classname'] ?> implements \byrokrat\banking\Format\FormatInterface
{
    use \byrokrat\banking\Format\ValidatorJitCache;

    public function getBankName(): string
    {
        return \byrokrat\banking\BankNames::<?= $data['bank'] ?>;
    }

    protected function getClearingValidator(): \byrokrat\banking\Validator\ValidatorInterface
    {
        return new \byrokrat\banking\Validator\<?= $data['clearing-validator'] ?>;
    }
    ...
    {
        return [
<?php foreach ($data['additional-validators'] as $validator) { ?>
            new \byrokrat\banking\Validator\<?= $validator ?>,
<?php } ?>
        ];
    }
}
